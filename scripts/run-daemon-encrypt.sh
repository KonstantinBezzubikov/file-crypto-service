...
LOG_FILE="${SCRIPT_DIR}/gpb-file-crypto-service-encrypt.log"

nohup java -jar "${JAR_PATH}"   -action encrypt   -mode daemon   -in "${INPUT_DIR}"   -out "${OUTPUT_DIR}"   -conf "${CONF_FILE}"   > "${LOG_FILE}" 2>&1 &
