...
# Одноразовая расшифровка одного файла.
# В текущей stub-версии не требуется специальный формат входа, но файлы .enc/.dec/.in_progress
# при обработке папок будут пропущены фильтром.
INPUT_FILE="/opt/gpb/filecrypto/input/report.txt"
OUTPUT_FILE=""
CRL_FILE=""

CMD=(java -jar "${JAR_PATH}" -action decrypt -mode default -in "${INPUT_FILE}")
